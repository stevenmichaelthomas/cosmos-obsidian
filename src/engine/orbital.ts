...
import { hashN, hashFloat, countSyllables, fnv1a, contentId } from './hash';
...
export function generateOrbital(
  ...
  const h = hashN(seed, 12);

  const kind = deriveKind(contentType, contentStr, content);

  const mScore = contentType === 'haiku'
  ...
  const colorIdx = fnv1a(`color:${contentStr}`) % BODY_COLORS.length;
